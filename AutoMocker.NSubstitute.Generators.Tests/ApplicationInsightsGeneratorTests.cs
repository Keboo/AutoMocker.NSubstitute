...
using VerifyCS = AutoMocker.NSubstitute.Generators.Tests.CSharpSourceGeneratorVerifier<AutoMocker.NSubstitute.Generators.ApplicationInsightsExtensionSourceGenerator>;
...
public class ApplicationInsightsGeneratorTests
{
    private const string ExpectedApplicationInsightsV3GeneratedFile = """
        #nullable enable
        // Generated by AutoMocker.NSubstitute.Generators.ApplicationInsightsExtensionSourceGenerator.
    ...
    public async Task WhenGeneratorIsDisabled_NoGenerationOccurs()
    {
        ...
        test.SetGlobalOption("build_property.EnableAutoMockerNSubstituteApplicationInsightsGenerator", "false");
    ...
    public async Task WhenGeneratorIsExplicitlyEnabled_ExtensionMethodIsGenerated()
    {
        ...
        test.SetGlobalOption("build_property.EnableAutoMockerNSubstituteApplicationInsightsGenerator", "true");
        await test.RunAsync(TestContext.CancellationToken);
    }

    private static (string FileName, SourceText SourceText) GetSourceFile(string content, string fileName)
    {
        return (Path.Combine("AutoMocker.NSubstitute.Generators", "AutoMocker.NSubstitute.Generators.ApplicationInsightsExtensionSourceGenerator", fileName), SourceText.From(content, Encoding.UTF8, SourceHashAlgorithm.Sha256));
    }
