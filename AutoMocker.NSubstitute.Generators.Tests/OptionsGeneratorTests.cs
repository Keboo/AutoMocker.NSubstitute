...
using VerifyCS = AutoMocker.NSubstitute.Generators.Tests.CSharpSourceGeneratorVerifier<AutoMocker.NSubstitute.Generators.OptionsExtensionSourceGenerator>;
...
public class OptionsGeneratorTests
{
    private const string ExpectedOptionsGeneratedFile = """
        #nullable enable
        // Generated by AutoMocker.NSubstitute.Generators.OptionsExtensionSourceGenerator.
    ...
    public async Task WhenGeneratorIsDisabled_NoGenerationOccurs()
    {
        ...
        test.SetGlobalOption("build_property.EnableAutoMockerNSubstituteOptionsGenerator", "false");
    ...
    public async Task WhenGeneratorIsExplicitlyEnabled_ExtensionMethodIsGenerated()
    {
        ...
        test.SetGlobalOption("build_property.EnableAutoMockerNSubstituteOptionsGenerator", "true");
        await test.RunAsync(TestContext.CancellationToken);
    }

    private static (string FileName, SourceText SourceText) GetSourceFile(string content, string fileName)
    {
        return (Path.Combine("AutoMocker.NSubstitute.Generators", "AutoMocker.NSubstitute.Generators.OptionsExtensionSourceGenerator", fileName), SourceText.From(content, Encoding.UTF8, SourceHashAlgorithm.Sha256));
    }
