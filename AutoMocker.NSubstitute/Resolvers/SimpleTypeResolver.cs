...
public abstract class SimpleTypeResolver<T> : ISubstituteResolver
{
    private static Lazy<HashSet<Type>> Interfaces { get; } = new(() => [.. typeof(T).GetInterfaces()]);

    /// <summary>
    /// A flag indicating if <see cref="GetValue(SubstituteResolutionContext)"/> should be called if the the requested type is a base type of <typeparamref name="T"/>. Defaults to true.
    /// </summary>
    protected bool IncludeBaseTypes { get; set; } = true;

    /// <summary>
    /// A flag indicating if <see cref="GetValue(SubstituteResolutionContext)"/> should be called if the the requested type is an interface implemented by <typeparamref name="T"/>. Defaults to true.
    /// </summary>
    protected bool IncludeInterfaces { get; set; } = true;

    /// <inheritdoc />
    public virtual void Resolve(SubstituteResolutionContext context)
    {
    ...
    protected abstract T GetValue(SubstituteResolutionContext context);
}
