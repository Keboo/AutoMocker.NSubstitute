...
public static class Diagnostics
{
    ...
    public static class MustReferenceAutoMock
    {
        ...
        private const string MessageFormat = $"To use AutoMocker.NSubstitute.Generators, your test project must also reference {AutoMock.AssemblyName}";
        private const string Description = $"Add a reference to the {AutoMock.AssemblyName} assembly or reference the {AutoMock.NuGetPackageName} NuGet package by running \"Install-Package AutoMocker.NSubstitute\".";
    ...
    public static class MustSpecifyTargetType
    {
        ...
        private const string MessageFormat = $"Class {{0}} is decorated with {AutoMock.ConstructorTestsAttribute} attribute but it does not specify a {AutoMock.TargetTypePropertyName}. Set this property to the type you want to generate tests for: [NSubstitute.AutoMock.ConstructorTests(TargetType = typeof(TypeToGenerateTestsFor))].";
