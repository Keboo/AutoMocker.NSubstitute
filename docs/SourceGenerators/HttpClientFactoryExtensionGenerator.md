...
- Provides a testable `IHttpClientFactory` backed by the same `HttpMessageHandlerWrapper` substitute used by `HttpClientResolver`
...
4. Every cached `HttpClient` wraps the same testable handler used by `HttpClientResolver`, so requests made through any named client are visible to the existing `SetupHttp*`/`VerifyHttp*` helpers
...
```xml
<PropertyGroup>
  <EnableAutoMockerNSubstituteHttpClientFactoryGenerator>false</EnableAutoMockerNSubstituteHttpClientFactoryGenerator>
</PropertyGroup>
```
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    <PackageReference Include="AutoMocker.NSubstitute" Version="1.0.0" />
  </ItemGroup>
</Project>
```
...
4. Ensure you're using the `NSubstitute.AutoMock` namespace
